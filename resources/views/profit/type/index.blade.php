<x-app-layout>
    ...
    <style>
        /* Inputs */
        .sheet-input { width: 100%; background: transparent; border: 1px solid transparent; padding: 4px 8px; border-radius: 4px; outline: none; font-size: 0.875rem; transition: all 0.2s; }
        .sheet-input:focus { background-color: white; border-color: #6366f1; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.1); }
        .sheet-input[readonly] { cursor: default; color: #475569; }
        
        /* Selects */
        .sheet-select { width: 100%; background: transparent; border: 1px solid transparent; padding: 4px 8px; border-radius: 4px; outline: none; font-size: 0.875rem; cursor: pointer; }
        .sheet-select:focus { background-color: white; border-color: #6366f1; }
    ...
    <div x-data="{ 
        editingId: null,
        cols: { id: true, code: true, parent: true, name: true, branch: true, creator: true, desc: true, active: true, actions: true },
        ...
        <div class="mx-6 mb-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('profit.types.trash') }}" class="w-10 h-10 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-100 border border-red-100 transition" title="{{ __('profit.trash') }}">
                ...
                <div x-data="{ openDropdown: false }" class="relative">
                    ...
                    <div x-show="openDropdown" class="absolute top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-2 ltr:right-0 rtl:left-0" x-cloak style="display:none;">
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            @foreach(['id'=>'#', 'code'=>__('profit.code'), 'parent'=>__('profit.parent_group'), 'name'=>__('profit.name'), 'branch'=>__('profit.branch'), 'creator'=>__('profit.created_by'), 'desc'=>__('profit.description'), 'active'=>__('profit.active')] as $key => $label)
                ...
                <a href="{{ route('profit.types.pdf') }}" target="_blank" class="w-10 h-10 flex items-center justify-center rounded-lg bg-slate-700 text-white hover:bg-slate-800 transition shadow-lg shadow-slate-300/50">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                </a>
                
                <button type="button" @click="addNewType()" class="w-10 h-10 flex items-center justify-center rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition">
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-slate-500 table-fixed whitespace-nowrap">
                        <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100">
                            <tr>
                                ...
                                <th x-show="cols.parent" class="px-4 py-3 w-[15%]">{{ __('profit.parent_group') }}</th>
                                <th x-show="cols.name" class="px-4 py-3 w-[18%]">{{ __('profit.name') }}</th>
                                ...
                                <th x-show="cols.desc" class="px-4 py-3 w-[20%]">{{ __('profit.description') }}</th>
                        ...
                        <tbody id="body-types" class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors group/row" :class="editingId === {{ $type->id }} ? 'bg-indigo-50/20' : ''">
                                ...
                                {{-- PARENT GROUP --}}
                                <td x-show="cols.parent" class="px-2 py-1">
                                    <span x-show="editingId !== {{ $type->id }}" class="block px-2 text-slate-600 truncate">{{ $type->group->name ?? '-' }}</span>
                                ...
                                <td x-show="cols.name" class="px-2 py-1">
                                    <span x-show="editingId !== {{ $type->id }}" class="block px-2 text-slate-700 font-bold truncate">{{ $type->name }}</span>
                                    <input x-show="editingId === {{ $type->id }}" type="text" name="types[{{ $index }}][name]" value="{{ $type->name }}" class="sheet-input font-bold">
                                </td>

                                {{-- BRANCH (Fix: Show 'Main Branch' if null) --}}
                                <td x-show="cols.branch" class="px-2 py-1">
                                    <span x-show="editingId !== {{ $type->id }}" class="block px-2 text-slate-600 font-medium truncate">
                                        {{ $type->branch->name ?? 'Main Branch' }}
                                    </span>
                                    <select x-show="editingId === {{ $type->id }}" name="types[{{ $index }}][branch_id]" class="sheet-select w-full">
                                ...
                                <td x-show="cols.creator" class="px-2 py-1">
                                    <span class="block px-2 text-slate-500 text-xs truncate">{{ $type->creator->name ?? 'System' }}</span>
                                </td>

                                {{-- DESC --}}
                                <td x-show="cols.desc" class="px-2 py-1">
                                    <span x-show="editingId !== {{ $type->id }}" class="block px-2 text-slate-500 truncate" title="{{ $type->description }}">{{ $type->description }}</span>
                                ...
                                <td x-show="cols.active" class="px-2 py-1 text-center">
                                    <input type="checkbox" name="types[{{ $index }}][is_active]" value="1" {{ $type->is_active ? 'checked' : '' }} :disabled="editingId !== {{ $type->id }}" class="w-4 h-4 text-indigo-600 rounded border-slate-300 cursor-pointer disabled:opacity-50">
                                </td>
                                ...
                                <td x-show="cols.actions" class="px-2 py-1 text-center print:hidden">
                                    <div class="flex items-center justify-center h-8 gap-1">
                                        ...
                                        <div x-show="editingId !== {{ $type->id }}" class="flex items-center gap-1">
    ...
    <script>
        function addNewType() {
            ...
            // --- FIX: USE 'Main Branch' IF USER BRANCH IS NULL ---
            const userBranchName = "{{ optional(auth()->user()->branch)->name ?? 'Main Branch' }}";
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                ...
                <td x-show="cols.parent" class="px-2 py-1">
                    <select name="types[${index}][profit_group_id]" class="sheet-select w-full font-bold" autofocus>${groupOptions}</select>
                </td>

                <td x-show="cols.name" class="px-2 py-1">
                    <input type="text" name="types[${index}][name]" class="sheet-input font-bold" placeholder="{{ __('profit.name') }}">
                </td>
                
                <td x-show="cols.branch" class="px-2 py-1">
                    <input type="text" value="${userBranchName}" class="sheet-input text-slate-600 cursor-default" readonly>
                    <input type="hidden" name="types[${index}][branch_id]" value="${userBranchId}">
                </td>
                ...
                <td x-show="cols.desc" class="px-2 py-1">
                    <input type="text" name="types[${index}][description]" class="sheet-input" placeholder="...">
                </td>

                <td x-show="cols.active" class="px-2 py-1 text-center">
                    <input type="checkbox" name="types[${index}][is_active]" value="1" checked class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer">
                </td>
                
                <td x-show="cols.actions" class="px-2 py-1 text-center print:hidden">
                    <div class="flex items-center justify-center h-8 gap-1">
                        <button type="button" onclick="document.getElementById('form-types').submit()" class="w-7 h-7 flex items-center justify-center bg-emerald-500 text-white rounded shadow hover:bg-emerald-600 transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                        </button>
            ...
            setTimeout(() => { tableBody.lastElementChild.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 100);
        }
