<x-app-layout>
    ...
    <div x-data="{ 
        hasChanges: false,
        editingId: null,
        cols: { id: true, code: true, name: true, group: true, branch: true, creator: true, desc: true, created_at: true, active: true, actions: true },
        ...
        <div class="mx-4 mb-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('profit.types.trash') }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-50 text-red-500 border border-red-100 transition shadow-sm">
                ...
                <div x-data="{ openDropdown: false }" class="relative">
                    ...
                    <div x-show="openDropdown" class="absolute ltr:right-0 rtl:left-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-2" x-cloak>
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            @foreach(['id'=>'#', 'code'=>__('profit.code'), 'name'=>__('profit.name'), 'group'=>__('profit.group_title'), 'branch'=>__('profit.branch'), 'creator'=>__('profit.created_by'), 'desc'=>__('profit.description'), 'created_at'=>__('profit.created_at'), 'active'=>__('profit.active')] as $key => $label)
            ...
            <form id="form-types" action="{{ route('profit.types.store') }}" method="POST">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-slate-500 whitespace-nowrap">
                    ...
                    <tbody id="body-types" class="divide-y divide-slate-100">
                        ...
                        <tr class="bg-white hover:bg-slate-50 transition-colors group/row" :class="editingId === {{ $type->id }} ? 'bg-indigo-50/20' : ''">
                            ...
                            <td x-show="cols.code" class="p-1">
                                <span class="px-3 block text-slate-500 uppercase font-normal">{{ $type->code }}</span>
                            </td>
                            ...
                            <td x-show="cols.group" class="p-1">
                                ...
                                <select x-show="editingId === {{ $type->id }}" name="types[{{ $index }}][group_spending_id]" class="sheet-input font-normal">
                            ...
                            <td x-show="cols.desc" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-3 block text-slate-500 italic font-normal truncate">{{ $type->note ?? '-' }}</span>
                                <input x-show="editingId === {{ $type->id }}" type="text" name="types[{{ $index }}][note]" value="{{ $type->note }}" class="sheet-input font-normal">
                            </td>
    ...
    <script>
        function addNewType() {
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                ...
                <td x-show="cols.group" class="p-1"><select name="types[${index}][group_spending_id]" class="sheet-input font-normal">${groupOptions}</select></td>
                ...
                <td x-show="cols.desc" class="p-1"><input type="text" name="types[${index}][note]" class="sheet-input font-normal" placeholder="..."></td>
