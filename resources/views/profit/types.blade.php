<x-app-layout>
    <x-slot name="header">
        {{ __('profit.types_title') }}
    </x-slot>

    {{-- EXACT SAME SPREADSHEET STYLES --}}
    <style>
    ...
    <div x-data="{ hasChanges: false }" class="py-6" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            <h3 class="text-xl font-black text-slate-800 tracking-tight">{{ __('profit.types_title') }}</h3>
        ...
        <div class="overflow-x-auto pb-4 rounded-xl border border-slate-200 bg-white shadow-sm mx-4">
            <form id="sheet-form" action="{{ route('profit.types.store') }}" method="POST">
                @csrf
                <table class="w-full min-w-[800px] text-sm text-left text-slate-500">
                    <thead class="text-xs text-slate-700 uppercase bg-slate-100 border-b-2 border-slate-200 h-12">
                        <tr>
                            ...
                            <th class="min-w-[150px] px-2">{{ __('profit.parent_group') }}</th>
                    ...
                    <tbody id="sheet-body">
                        ...
                        <tr class="group transition-colors">
                            ...
                            <td>
                                <select name="types[{{ $index }}][profit_group_id]" class="sheet-input font-normal cursor-pointer bg-slate-50/50">
                            ...
                            <td><input type="text" name="types[{{ $index }}][name]" value="{{ $type->name }}" class="sheet-input font-bold text-slate-800" placeholder="{{ __('profit.name') }}"></td>
                            <td><input type="text" name="types[{{ $index }}][description]" value="{{ $type->description }}" class="sheet-input" placeholder="..."></td>
                            <td class="text-center bg-slate-50/30">
                                <div class="flex items-center justify-center h-full">
                                    <input type="checkbox" name="types[{{ $index }}][is_active]" value="1" {{ $type->is_active ? 'checked' : '' }} class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer">
                                </div>
                            </td>
                            <td class="text-center print:hidden bg-slate-50/30">
                                <div class="flex items-center justify-center h-full">
                                    <button type="button" onclick="deleteRow({{ $type->id }})" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
        ...
        function addNewRow() {
            ...
            const row = `
                <tr class="group bg-blue-50/20 hover:bg-blue-50 transition-colors animate-pulse-once">
                    ...
                    <td><select name="types[${index}][profit_group_id]" class="sheet-input font-normal cursor-pointer bg-slate-50/50">${groupOptions}</select></td>
                    <td><input type="text" name="types[${index}][name]" class="sheet-input font-bold" placeholder="{{ __('profit.name') }}" autofocus></td>
                    <td><input type="text" name="types[${index}][description]" class="sheet-input" placeholder="..."></td>
                    <td class="text-center bg-slate-50/30"><div class="flex items-center justify-center h-full"><input type="checkbox" name="types[${index}][is_active]" value="1" checked class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"></div></td>
