<x-app-layout>
    <x-slot name="header">
        {{ __('profit.menu_groups') }} - {{ __('profit.trash') ?? 'Trash' }}
    </x-slot>

    <style>@media print { .no-print, button, a { display: none !important; } }</style>

    <div class="py-6 w-full min-w-0" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            <div class="flex items-center gap-3">
                <div class="p-2 bg-red-100 rounded-lg text-red-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                </div>
                <h3 class="text-xl font-black text-slate-800 tracking-tight">{{ __('profit.menu_groups') }} ({{ __('profit.trash') ?? 'Trash' }})</h3>
            </div>
            
            {{-- FIXED LINK: Changed from profit.index to profit.groups.index --}}
            <a href="{{ route('profit.groups.index') }}" class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 hover:text-indigo-600 transition shadow-sm font-medium">
                <svg class="w-4 h-4 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                <span>{{ __('profit.back') ?? 'Back' }}</span>
            </a>
        </div>

        <div class="relative overflow-x-auto bg-white shadow-sm rounded-xl border border-slate-200 mx-4">
            <table class="w-full text-sm text-left rtl:text-right text-slate-500">
                <thead class="text-xs text-slate-700 uppercase bg-red-50/50 border-b border-red-100">
                    <tr>
                        <th class="px-6 py-3 font-bold text-center w-16">#</th>
                        <th class="px-6 py-3 font-bold">{{ __('profit.name') }}</th>
                        <th class="px-6 py-3 font-bold text-center">{{ __('profit.deleted_by') }}</th>
                        <th class="px-6 py-3 font-bold text-center">{{ __('profit.deleted_at') }}</th>
                        <th class="px-6 py-3 font-bold text-center w-32">{{ __('profit.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($groups as $group)
                    <tr class="bg-white hover:bg-red-50/30 transition-colors">
                        <td class="px-6 py-4 text-center font-medium text-slate-400">{{ $loop->iteration }}</td>
                        <td class="px-6 py-4 font-bold text-slate-800">{{ $group->name }}</td>
                        <td class="px-6 py-4 text-center">
                            @if($group->deleter)
                                <span class="text-xs font-bold text-slate-700">{{ $group->deleter->name }}</span>
                            @else
                                <span class="text-xs text-slate-400 italic">System</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center text-xs text-red-500 font-medium">
                            {{ $group->deleted_at->format('Y-m-d H:i') }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('profit.groups.restore', $group->id) }}" method="POST">@csrf <button type="submit" class="p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg transition-colors shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg></button></form>
                                <button onclick="confirmDelete('{{ route('profit.groups.force-delete', $group->id) }}')" class="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors shadow-sm"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-slate-400 italic">{{ __('profit.trash_empty') ?? 'Trash is empty' }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="mt-4 px-4">{{ $groups->links() }}</div>
    </div>
    
    <form id="force-delete-form" action="" method="POST" class="hidden">@csrf @method('DELETE')</form>
    
    <script>
        function confirmDelete(url) {
            if(confirm("{{ __('profit.confirm_delete') ?? 'Are you sure?' }}")) {
                document.getElementById('force-delete-form').action = url;
                document.getElementById('force-delete-form').submit();
            }
        }
    </script>
</x-app-layout>