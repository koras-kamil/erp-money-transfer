<x-app-layout>
    ...
    <style>
        ...
        select.sheet-input {
            ...
            padding-right: 2.5rem; padding-left: 0.75rem; cursor: pointer;
        }
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex flex-wrap items-center gap-2">
                {{-- Trash --}}
                <a href="{{ route('currency.trash') }}" class="w-9 h-9 flex items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition border border-red-100" title="{{ __('currency.trash') }}">
                ...
                <div x-data="{ openDropdown: false }" class="relative">
                    ...
                    <div x-show="openDropdown" class="absolute top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-2 ltr:right-0 rtl:left-0" x-cloak>
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            @foreach([
                                'id'     => '#', 
                                'type'   => __('currency.type'), 
                                'symbol' => __('currency.symbol'), 
                                'digit'  => __('currency.digit'), 
                                'total'  => __('currency.price_total'), 
                                'single' => __('currency.price_single'), 
                                'branch' => __('currency.branch'), 
                                'active' => __('currency.active')
                            ] as $key => $label)
                ...
                {{-- Print --}}
                <a href="{{ route('currency.print') }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-700 text-white hover:bg-slate-800 transition shadow-md shadow-slate-300/50" title="{{ __('currency.print') }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                </a>

                {{-- Add New --}}
                <button type="button" @click="addNewRow()" class="w-9 h-9 flex items-center justify-center rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition" title="{{ __('currency.add_new') }}">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-slate-500" @input="hasChanges = true" @change="hasChanges = true">
                    <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100">
                        <tr>
                            ...
                            <th x-show="cols.branch" class="px-4 py-3 w-[15%]">{{ __('currency.branch') }}</th>
                    ...
                    <tbody id="sheet-body" class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors group/row"
                            ...
                            <td x-show="cols.type" class="p-1">
                                <span x-show="editingId !== {{ $currency->id }}" class="px-2 block text-slate-800 font-bold uppercase">{{ $currency->currency_type }}</span>
                            ...
                            <td x-show="cols.symbol" class="p-1">
                                <span x-show="editingId !== {{ $currency->id }}" class="block text-center text-slate-600">{{ $currency->symbol }}</span>
                            ...
                            <td x-show="cols.digit" class="p-1">
                                <span x-show="editingId !== {{ $currency->id }}" class="block text-center text-slate-600">{{ $currency->digit_number }}</span>
                            ...
                            <td x-show="cols.single" class="p-1">
                                ...
                                <input x-show="editingId === {{ $currency->id }}" 
                                       ...
                                       value="{{ number_format($currency->price_single, 0) }}" 
                                       readonly
                                       class="sheet-input text-center font-normal text-blue-600 bg-slate-50 price-single">
                            </td>

                            <td x-show="cols.branch" class="p-1">
                                <span x-show="editingId !== {{ $currency->id }}" class="px-2 block text-xs uppercase text-slate-500">{{ optional($currency->branch)->name ?? '-' }}</span>
                            ...
                            <td x-show="cols.active" class="px-4 py-2 text-center">
                                <div class="flex items-center justify-center">
                                    <input type="checkbox" name="currencies[{{ $index }}][is_active]" value="1" {{ $currency->is_active ? 'checked' : '' }} :disabled="editingId !== {{ $currency->id }}" class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer disabled:opacity-50">
                                </div>
                            </td>
                            ...
                            <td x-show="cols.actions" class="px-4 py-2 text-center print:hidden">
                                <div class="relative h-8 flex items-center justify-center w-full">
                                    <div x-show="editingId === {{ $currency->id }}" class="absolute inset-0 flex items-center justify-center">
                                        <button type="button" @click="saveRow()" class="w-7 h-7 flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg shadow-sm transition transform active:scale-95">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                                        </button>
                                    </div>
                                    <div x-show="editingId !== {{ $currency->id }}" class="flex items-center justify-center gap-1">
                                        <button type="button" @click="startEdit({{ $currency->id }})" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                                        <button type="button" onclick="deleteDatabaseRow({{ $currency->id }})" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    <script>
        ...
        function addNewRow() {
            ...
            const labelNew = "{{ __('currency.new') }}";
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                ...
                <td x-show="cols.type" class="p-1"><input type="text" name="currencies[${index}][currency_type]" class="sheet-input font-normal uppercase" placeholder="${placeholderType}" autofocus></td>
                ...
                <td x-show="cols.active" class="px-4 py-2 text-center"><div class="flex items-center justify-center"><input type="checkbox" name="currencies[${index}][is_active]" value="1" checked class="w-4 h-4 text-indigo-600 rounded border-slate-300 cursor-pointer"></div></td>
