            ...
            <div class="flex items-center gap-4 w-full md:w-auto">
                <div class="bg-slate-100 p-1 rounded-lg flex items-center shadow-inner">
                    <span class="px-5 py-2 text-sm font-bold rounded-md bg-white text-indigo-600 shadow-sm transition">
                        {{-- 🟢 CHANGED TITLE TO PAYING --}}
                        {{ __('accountant.paying_title') }}
                    </span>
                </div>
                                ...
                                <tr class="hover:bg-indigo-50/20 transition-colors group">
                                    ...
                                    <td class="px-2 py-1 text-center sticky right-0 bg-white group-hover:bg-indigo-50/20 shadow-l z-10 border-b border-slate-100">
                                        <div class="flex items-center justify-center gap-1">
                                            {{-- 🟢 UPDATED ROUTES TO PAYING --}}
                                            <a :href="`/accountant/paying/${trx.id}/edit`" class="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors" title="{{ __('Edit') }}">
    ...
    <script>
        function accountantManager() {
            return {
                ...
                form: { amount: '', currency_id: '', rate: 1, discount: 0 },
                init() { if (!this.accounts) this.accounts = []; this.columns.forEach(c => this.filters[c.field] = ''); if (this.currencies.length > 0) { this.setCurrency(this.currencies[0].id); } },
                setCurrency(id) { this.form.currency_id = id; const selected = this.currencies.find(c => c.id == id); if (selected) { this.form.rate = selected.price_sell || 1; } },
                ...
                get filteredTransactions() { let data = this.transactions.filter(trx => { for (const col of this.columns) { const filterVal = this.filters[col.field]?.toLowerCase(); if (!filterVal) continue; let cellVal = ''; if (col.field === 'user') cellVal = trx.account?.name || ''; else if (col.field === 'cashbox') cellVal = trx.cashbox?.name || ''; else if (col.field === 'user_id') cellVal = trx.user?.name || ''; else cellVal = String(trx[col.field] || ''); if (!cellVal.toLowerCase().includes(filterVal)) return false; } return true; }); if (this.sortCol) { data.sort((a, b) => { let valA = String(a[this.sortCol] || '').toLowerCase(); let valB = String(b[this.sortCol] || '').toLowerCase(); if (valA < valB) return this.sortAsc ? -1 : 1; if (valA > valB) return this.sortAsc ? 1 : -1; return 0; }); } return data; },
