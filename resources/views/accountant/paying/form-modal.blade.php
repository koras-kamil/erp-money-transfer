            ...
            <div class="bg-slate-50 px-4 py-2 flex justify-between items-center border-b border-slate-100 flex-shrink-0">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-xs font-black text-slate-800 uppercase tracking-widest">
                        <span x-text="isEditing ? '{{ __('Edit Payment') }} #' + editingId : '{{ __('accountant.new_payment') }}'"></span>
                    </h3>
                </div>
            ...
            <form :action="isEditing ? '/accountant/paying/' + editingId : '{{ route('accountant.paying.store') }}'" 
                ...
                <div class="border border-slate-300 mb-4 bg-white">
                    <div class="grid grid-cols-12 border-b border-slate-200">
                        ...
                        <div class="col-span-12 md:col-span-6 p-1 flex items-center"><label class="w-24 md:w-24 text-xs font-bold text-slate-500 uppercase text-center">{{ __('accountant.price_usd') }}</label><input type="text" name="exchange_rate" x-model="form.rate" @input="formatRateInput(); calculateTotal()" class="flex-1 h-9 text-base font-bold border border-slate-300 rounded-md bg-white focus:ring-0 text-center p-0 placeholder-slate-300"></div>
                    </div>
                    <div class="grid grid-cols-12 border-b border-slate-200">
                        <div class="col-span-12 p-1 flex items-center"><label class="w-28 md:w-40 text-xs font-bold text-slate-600 uppercase text-center">{{ __('accountant.amount_pay') }}</label><input type="text" name="amount" x-model="form.amount" @input="formatNumber('amount'); calculateTotal()" class="flex-1 h-10 text-lg font-black border border-slate-300 rounded-md bg-white focus:ring-0 text-center p-0 text-slate-800" placeholder="0.00"></div>
                    </div>
                    <div class="grid grid-cols-12 border-b border-slate-200">
                        <div class="col-span-12 p-1 flex items-center"><label class="w-28 md:w-40 text-xs font-bold text-slate-500 uppercase text-center">{{ __('accountant.discount') }}</label><input type="text" x-model="form.discount" name="discount" @input="formatNumber('discount'); calculateTotal()" class="flex-1 h-9 text-base font-bold border border-slate-300 rounded-md bg-white focus:ring-0 text-center p-0 text-rose-500" placeholder="0"></div>
                    </div>
                    <div class="grid grid-cols-12 border-b border-slate-200">
                        <div class="col-span-12 p-1 flex items-center"><label class="w-28 md:w-40 text-xs font-bold text-emerald-700 uppercase text-center">{{ __('accountant.cash_box') }}</label><select name="cashbox_id" x-model="form.cashbox_id" class="flex-1 h-9 text-base font-bold border border-slate-300 rounded-md bg-white focus:ring-0 text-center p-0 text-emerald-800 cursor-pointer"><option value="" disabled selected>{{ __('Select') }}</option><template x-for="box in availableCashboxes" :key="box.id"><option :value="box.id" x-text="box.name"></option></template></select></div>
                    </div>
                    {{-- 🟢 TOTAL ROW --}}
                    <div class="grid grid-cols-12">
                        <div class="col-span-12 p-1 flex items-center"><label class="w-28 md:w-40 text-xs font-bold text-indigo-700 uppercase text-center">{{ __('accountant.total_after_discount') }}</label>
                            <div class="flex-1 flex items-center justify-center gap-2">
                                <input type="text" x-model="form.total" @input="formatNumber('total'); recalcRateFromTotal(); if($event.target.value === '') resetTotal();" :readonly="isTotalLocked" class="w-32 md:w-56 h-10 text-lg font-black border border-slate-300 rounded-md bg-white focus:ring-0 text-center p-0 text-indigo-800">
                ...
                <div class="mb-2">
                    ...
                    <div x-show="showProfit" class="border border-slate-300 bg-white">
                        <div class="grid grid-cols-12 border-b border-slate-300">
                            ...
                            <div class="col-span-4 p-1"><input type="text" name="profit_amount" @input="formatNumber('profit_amount')" class="w-full h-9 text-base font-bold border border-slate-300 rounded-md px-1 bg-white text-center focus:ring-0" placeholder="3000"></div>
                        </div>
                ...
                <div class="mb-4">
                    ...
                    <div x-show="showSpending" class="border border-slate-300 bg-white">
                        <div class="grid grid-cols-12 border-b border-slate-300">
                            ...
                            <div class="col-span-4 p-1"><input type="text" name="spending_amount" @input="formatNumber('spending_amount')" class="w-full h-9 text-base font-bold border border-slate-300 rounded-md px-1 bg-white text-center focus:ring-0" placeholder="1000"></div>
                        </div>
                        <div class="bg-slate-50 p-1 text-center text-xs text-slate-500 font-bold border-t border-slate-300">{{ __('accountant.debt') }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-4 gap-0 text-white text-xs font-bold text-center">
                    <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 py-3 transition-colors" x-text="isEditing ? '{{ __('accountant.update') }}' : '{{ __('accountant.save') }}'"></button>
