{{-- 🟢 PAYING FORM MODAL (Final: Profile Right, Close Left, Translatable Info) --}}
<div x-data="payingForm()" 
     @open-paying-modal.window="openModal($event.detail)"
     x-show="showModal" 
     x-cloak 
     class="fixed inset-0 z-50 overflow-hidden" dir="rtl">
    ...
    <div class="flex h-screen w-full items-center justify-center p-2">
        <div class="relative w-full max-w-2xl transform overflow-hidden rounded-xl bg-white shadow-2xl transition-all border border-slate-100 flex flex-col max-h-[95vh]">
            ...
            <div class="bg-slate-50 px-4 py-2 flex justify-between items-center border-b border-slate-100 flex-shrink-0">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-xs font-black text-slate-800 uppercase tracking-widest">
                        <span x-text="isEditing ? '{{ __('accountant.edit_payment') }} #' + editingId : '{{ __('accountant.new_payment') }}'"></span>
                    </h3>
                </div>
            ...
            <form :action="isEditing ? '/accountant/paying/' + editingId : '{{ route('accountant.paying.store') }}'" 
                ...
                <input type="hidden" name="type" value="pay"> {{-- Corrected to pay --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-2 mb-4 bg-white">
                    <div class="col-span-1 md:col-span-8 relative flex flex-col gap-1">
                        ...
                        <div class="relative w-full h-16" @click.away="searchOpen = false">
                            ...
                            {{-- State: No User Selected (Show Input) --}}
                                    ...
                                    <div @click="selectAccount(acc)" class="px-3 py-2.5 hover:bg-indigo-50 cursor-pointer text-sm font-bold border-b border-slate-50 last:border-0 flex items-center justify-between text-right gap-3">
                                        <div class="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div class="flex flex-col truncate">
                                                <span x-text="acc.name" class="truncate text-slate-700 text-sm"></span>
                                            </div>
                                        </div>
                        ...
                        <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg p-1.5 h-10 shadow-sm w-full" x-show="selectedAccount">
                            <div class="relative flex-shrink-0">
                                ...
                                <div x-show="showUserConfig" @click.away="showUserConfig = false" class="absolute top-full right-0 mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-50 p-2 text-xs text-right" style="display: none;">
                                    <div class="font-bold text-slate-400 mb-1 px-1 uppercase text-[10px]">{{ __('accountant.toggle_columns') }}</div>
                                    <label class="flex items-center gap-2 p-1.5 hover:bg-slate-50 rounded cursor-pointer"><input type="checkbox" x-model="userInfoVisible.code" class="rounded text-indigo-600 w-3.5 h-3.5"><span class="text-slate-600">{{ __('accountant.manual_code') }}</span></label>
                                    <label class="flex items-center gap-2 p-1.5 hover:bg-slate-50 rounded cursor-pointer"><input type="checkbox" x-model="userInfoVisible.city" class="rounded text-indigo-600 w-3.5 h-3.5"><span class="text-slate-600">{{ __('accountant.city') }}</span></label>
                                    <label class="flex items-center gap-2 p-1.5 hover:bg-slate-50 rounded cursor-pointer"><input type="checkbox" x-model="userInfoVisible.neighborhood" class="rounded text-indigo-600 w-3.5 h-3.5"><span class="text-slate-600">{{ __('accountant.neighborhood') }}</span></label>
                                    <label class="flex items-center gap-2 p-1.5 hover:bg-slate-50 rounded cursor-pointer"><input type="checkbox" x-model="userInfoVisible.mobile" class="rounded text-indigo-600 w-3.5 h-3.5"><span class="text-slate-600">{{ __('accountant.giver_mobile') }}</span></label>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 flex items-center gap-3 overflow-x-auto custom-scrollbar px-1 text-xs font-medium text-slate-600 whitespace-nowrap">
                                ...
                                <div x-show="userInfoVisible.city && selectedAccount?.city_name" class="flex items-center gap-1"><span class="text-slate-400">{{ __('accountant.city') }}:</span> <span x-text="selectedAccount?.city_name"></span></div>
                                <div x-show="userInfoVisible.neighborhood && selectedAccount?.neighborhood_name" class="flex items-center gap-1 border-r border-slate-200 pr-2 mr-2"><span class="text-slate-400">{{ __('accountant.neighborhood') }}:</span> <span x-text="selectedAccount?.neighborhood_name"></span></div>
                        ...
                        <div class="flex flex-col gap-2 w-full">
                            <template x-for="curr in availableCurrencies" :key="curr.id">
                                <div @click="target_currency_id = curr.id; updateRate()" 
                                    ...
                                    {{-- 🟢 LIVE BALANCE WITH SMART COLORS (RED = DEBT, GREEN = LOAN) --}}
                                    <div class="flex flex-col items-start">
                                        ...
                                        <span class="text-[10px] opacity-80" x-text="(selectedAccount?.balances?.[curr.id] < 0) ? 'قەرز' : 'قەرزارە'"></span>
                                    </div>
                ...
                <div class="mb-2">
                    <label class="flex items-center gap-2 cursor-pointer mb-1 bg-emerald-50/30 border border-emerald-200 p-1.5 rounded w-fit">
                        ...
                        <span class="text-xs font-black text-emerald-800 uppercase px-1">{{ __('accountant.profit') }}</span>
                    </label>
                    <div x-show="showProfit" x-collapse class="border-2 border-emerald-400 bg-white rounded-md shadow-sm">
                        <div class="grid grid-cols-12 border-b border-emerald-200">
                            <div class="col-span-2 border-l border-emerald-200 p-0 flex items-center justify-center bg-emerald-50/30"><label class="text-[9px] font-bold text-emerald-800 text-center">{{ __('accountant.amount') }}</label></div>
                            ...
                            <div class="col-span-2 border-l border-r border-emerald-200 p-0 flex items-center justify-center bg-emerald-50/30"><label class="text-[9px] font-bold text-emerald-800 text-center">{{ __('accountant.type') }}</label></div>
                            <div class="col-span-4 p-0"><select name="profit_category_id" x-model="form.profit_category_id" @change="autoSelectProfitCurrency()" class="w-full h-9 text-xs font-bold border-0 bg-white text-center cursor-pointer text-slate-700"><option value="">-- {{ __('accountant.type') }} --</option><template x-for="cat in profitTypes" :key="cat.id"><option :value="cat.id" x-text="cat.name"></option></template></select></div>
                        </div>
                        <div class="grid grid-cols-12 border-b border-emerald-200">
                            <div class="col-span-2 border-l border-emerald-200 p-0 flex items-center justify-center bg-emerald-50/30"><label class="text-[9px] font-bold text-emerald-800 text-center">{{ __('accountant.currency') }}</label></div>
                            <div class="col-span-4 p-0"><select name="profit_currency_id" x-model="form.profit_currency_id" @change="updateProfitCashboxes()" class="w-full h-9 text-xs font-bold border-0 bg-white text-center cursor-pointer text-slate-700"><option value="">-- {{ __('accountant.currency') }} --</option><template x-for="curr in currencies" :key="curr.id"><option :value="curr.id" x-text="curr.currency_type"></option></template></select></div>
                            <div class="col-span-2 border-l border-r border-emerald-200 p-0 flex items-center justify-center bg-emerald-50/30"><label class="text-[9px] font-bold text-emerald-800 text-center">{{ __('accountant.cashbox') }}</label></div>
                            <div class="col-span-4 p-0"><select name="profit_cashbox_id" x-model="form.profit_cashbox_id" :disabled="form.profit_is_debt" :class="form.profit_is_debt ? 'bg-slate-100 text-slate-400' : 'bg-white text-emerald-700'" class="w-full h-9 text-xs font-bold border-0 text-center cursor-pointer text-emerald-700"><option value="" x-text="form.profit_is_debt ? '{{ __('accountant.disabled') }}' : '-- {{ __('accountant.select') }} --'"></option><template x-for="box in profitFilteredCashboxes" :key="box.id"><option :value="box.id" x-text="box.name"></option></template></select></div>
                        </div>
                        <div class="grid grid-cols-12 bg-rose-50/30">
                            <div class="col-span-4 p-0 flex items-center justify-center bg-rose-100/30 border-l border-emerald-200"><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="profit_is_debt" x-model="form.profit_is_debt" value="1" class="rounded text-rose-500 w-3.5 h-3.5"><span class="text-[10px] font-black text-rose-700">{{ __('accountant.debt') }}</span></label></div>
                            <div class="col-span-8 p-0 relative bg-white"><div x-show="form.profit_is_debt" class="relative w-full h-full" @click.away="profitSearchOpen = false"><input type="text" x-model="profitSearchQuery" @click="profitSearchOpen = true" @input="profitSearchOpen = true" placeholder="{{ __('accountant.search_users') }}" class="w-full h-9 text-xs font-bold text-center border-0 focus:ring-0 text-rose-800"><div x-show="profitSearchOpen" class="absolute top-full left-0 w-full bg-white border border-slate-200 shadow-xl max-h-60 overflow-y-auto z-50 rounded-b-md custom-scrollbar"><template x-for="acc in filteredProfitAccounts" :key="acc.id"><div @click="profitAccount = acc; profitSearchQuery = acc.name; profitSearchOpen = false;" class="px-3 py-2 hover:bg-rose-50 cursor-pointer text-xs font-bold border-b border-slate-50 flex justify-between items-center text-right"><div><span x-text="acc.name"></span></div><span class="text-slate-400" x-text="acc.code"></span></div></template></div><div class="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] text-green-600 font-black" x-show="profitAccount">✓</div></div><div x-show="!form.profit_is_debt" class="w-full h-9 flex items-center justify-center text-[10px] text-slate-300 italic bg-slate-50">{{ __('accountant.debt_disabled') }}</div></div>
                        </div>
                    </div>
                </div>
                ...
                {{-- 🟢 4. SPENDING SECTION --}}
                <div class="mb-4">
                    <label class="flex items-center gap-2 cursor-pointer mb-1 bg-rose-50/30 border border-rose-200 p-1.5 rounded w-fit">
                        <input type="checkbox" x-model="showSpending" class="rounded text-rose-600 w-4 h-4 border-rose-300">
                        <span class="text-xs font-black text-rose-800 uppercase px-1">{{ __('accountant.spending') }}</span>
                    </label>
                    <div x-show="showSpending" x-collapse class="border-2 border-rose-400 bg-white rounded-md shadow-sm">
                        <div class="grid grid-cols-12 border-b border-rose-200">
                            <div class="col-span-2 border-l border-rose-200 p-0 flex items-center justify-center bg-rose-50/30"><label class="text-[9px] font-bold text-rose-800 text-center">{{ __('accountant.amount') }}</label></div>
                            ...
                            <div class="col-span-2 border-l border-r border-rose-200 p-0 flex items-center justify-center bg-rose-50/30"><label class="text-[9px] font-bold text-rose-800 text-center">{{ __('accountant.type') }}</label></div>
                            <div class="col-span-4 p-0"><select name="spending_category_id" x-model="form.spending_category_id" @change="autoSelectSpendingCurrency()" class="w-full h-9 text-xs font-bold border-0 bg-white text-center cursor-pointer text-slate-700"><option value="">-- {{ __('accountant.type') }} --</option><template x-for="cat in spendingTypes" :key="cat.id"><option :value="cat.id" x-text="cat.name"></option></template></select></div>
                        </div>
                        <div class="grid grid-cols-12 border-b border-rose-200">
                            <div class="col-span-2 border-l border-rose-200 p-0 flex items-center justify-center bg-rose-50/30"><label class="text-[9px] font-bold text-rose-800 text-center">{{ __('accountant.currency') }}</label></div>
                            <div class="col-span-4 p-0"><select name="spending_currency_id" x-model="form.spending_currency_id" @change="updateSpendingCashboxes()" class="w-full h-9 text-xs font-bold border-0 bg-white text-center cursor-pointer text-slate-700"><option value="">-- {{ __('accountant.currency') }} --</option><template x-for="curr in currencies" :key="curr.id"><option :value="curr.id" x-text="curr.currency_type"></option></template></select></div>
                            <div class="col-span-2 border-l border-r border-rose-200 p-0 flex items-center justify-center bg-rose-50/30"><label class="text-[9px] font-bold text-rose-800 text-center">{{ __('accountant.cashbox') }}</label></div>
                            <div class="col-span-4 p-0"><select name="spending_cashbox_id" x-model="form.spending_cashbox_id" :disabled="form.spending_is_debt" :class="form.spending_is_debt ? 'bg-slate-100 text-slate-400' : 'bg-white text-rose-700'" class="w-full h-9 text-xs font-bold border-0 text-center cursor-pointer text-rose-700"><option value="" x-text="form.spending_is_debt ? '{{ __('accountant.disabled') }}' : '-- {{ __('accountant.select') }} --'"></option><template x-for="box in spendingFilteredCashboxes" :key="box.id"><option :value="box.id" x-text="box.name"></option></template></select></div>
                        </div>
                        <div class="grid grid-cols-12 bg-rose-50/30">
                            <div class="col-span-4 p-0 flex items-center justify-center bg-rose-100/30 border-l border-rose-200"><label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="spending_is_debt" x-model="form.spending_is_debt" value="1" class="rounded text-rose-500 w-3.5 h-3.5"><span class="text-[10px] font-black text-rose-700">{{ __('accountant.debt') }}</span></label></div>
                            <div class="col-span-8 p-0 relative bg-white"><div x-show="form.spending_is_debt" class="relative w-full h-full" @click.away="spendingSearchOpen = false"><input type="text" x-model="spendingSearchQuery" @click="spendingSearchOpen = true" @input="spendingSearchOpen = true" placeholder="{{ __('accountant.search_users') }}" class="w-full h-9 text-xs font-bold text-center border-0 focus:ring-0 text-rose-800"><div x-show="spendingSearchOpen" class="absolute top-full left-0 w-full bg-white border border-slate-200 shadow-xl max-h-60 overflow-y-auto z-50 rounded-b-md custom-scrollbar"><template x-for="acc in filteredSpendingAccounts" :key="acc.id"><div @click="spendingAccount = acc; spendingSearchQuery = acc.name; spendingSearchOpen = false;" class="px-3 py-2 hover:bg-rose-50 cursor-pointer text-xs font-bold border-b border-slate-50 flex justify-between items-center text-right"><div><span x-text="acc.name"></span></div><span class="text-slate-400" x-text="acc.code"></span></div></template></div><div class="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] text-green-600 font-black" x-show="spendingAccount">✓</div></div><div x-show="!form.spending_is_debt" class="w-full h-9 flex items-center justify-center text-[10px] text-slate-300 italic bg-slate-50">{{ __('accountant.debt_disabled') }}</div></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="border border-slate-300 bg-white mb-4 p-2 rounded-md">
                    ...
                    <div class="mt-2">
                        <input name="note" class="w-full h-10 text-base text-center rounded-md border border-slate-200 focus:border-indigo-300 focus:ring-0 placeholder:text-slate-300 placeholder:font-light" placeholder="{{ __('accountant.note') }}">
                    </div>
                </div>
    ...
    <script>
        function payingForm() {
            return {
                ...
                profitTypes: @json($profitTypes ?? []),
                spendingTypes: @json($spendingTypes ?? []),
                ...
                openModal(detail) {
                    ...
                    if (detail && detail.id) {
                        ...
                        this.form = { amount: '', discount: '', currency_id: '', cashbox_id: '', rate: 1, total: 0, profit_is_debt: false, spending_is_debt: false, manual_date: new Date().toISOString().split('T')[0] };
                ...
                calculateTotal() { 
                    ...
                    if (this.form.currency_id == this.target_currency_id) {
                        let final = amt + discount;
                        this.form.total = final.toLocaleString('en-US', { maximumFractionDigits: 2 });
                        return;
                    }
                    ...
                    let converted = 0;
                    if (tPrice >= sPrice) { converted = amt * rate; } 
                    else { converted = (rate !== 0) ? (amt / rate) : 0; }
                    
                    let final = converted + discount;
                    this.form.total = final.toLocaleString('en-US', { maximumFractionDigits: 2 });
                },
