<div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" dir="rtl">
    ...
    <div class="flex h-screen w-full items-center justify-center p-3">
        {{-- 🟢 COMPACT WIDTH: max-w-2xl (Preserved) --}}
        <div class="relative w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all border border-slate-100 flex flex-col max-h-full">
            ...
            <form action="{{ route('accountant.store') }}" method="POST"
                  ...
                  x-data="{ 
                      ...
                      // 🟢 1. Update Rate (For Display Only)
                      updateRate() {
                          ...
                          if (s && t) {
                              ...
                              // Visual: Always show the ratio > 1 for cleaner UX (e.g., 1500)
                              // The actual calculation logic below handles the direction independently
                              if (tRate >= sRate) {
                      ...
                      // 🟢 2. UNIVERSAL CALCULATION LOGIC (USD BASE)
                      calculateTotal() {
                          if (!this.isTotalLocked) return;
                          ...
                          // 1️⃣ GET PRICE IN USD (Base Reference)
                          // Assuming price_single is 'Units per USD' (e.g. 1500 for IQD)
                          let sPriceUSD = 1 / parseFloat(s.price_single || 1);
                          let tPriceUSD = 1 / parseFloat(t.price_single || 1);

                          // 2️⃣ UNIVERSAL FORMULA: Target = (Amount * SourceUSD) / TargetUSD
                          let converted = (amt * sPriceUSD) / tPriceUSD;
                          
                          this.form.total = parseFloat((converted - discount).toFixed(2));
                      },

                      // 🟢 3. Recalculate Rate from Total (Reverse)
                      recalcRateFromTotal() {
                          ...
                          // Simply: Rate = Total / Amount (or Amount / Total depending on direction)
                          // We stick to the visual representation (Rate > 1)
                          if (effectiveTotal > amt) {
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-3 h-full">
                    ...
                    <div class="md:col-span-2 md:row-start-1 bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-center">
                        <div class="flex justify-between items-center mb-1">
                            <label class="text-[10px] font-bold text-slate-500">{{ __('accountant.select_account') }}</label>
                            <button type="button" @click="showCard = !showCard" class="text-[9px] text-indigo-500 hover:text-indigo-700 font-bold flex items-center gap-1">
                    ...
                    {{-- 🟢 LEFT (SPAN 2): User Info --}}
                    <div x-show="showCard" x-transition class="md:col-span-2 md:row-start-2 bg-indigo-50/60 border border-indigo-100 rounded-xl p-2.5 shadow-sm relative flex-1">
                    ...
                    {{-- 🟢 RIGHT (SPAN 3): TARGET --}}
                    <div class="md:col-span-3 md:row-span-2 md:col-start-3 md:row-start-1 bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col shadow-sm h-full">
                        <div class="bg-slate-50 border-b border-slate-100 px-2.5 py-1.5 flex justify-between items-center">
                    ...
                    {{-- 8️⃣ AMOUNT CARD --}}
                    <div class="md:col-span-5 bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-2 mb-1.5">
                            <div class="col-span-6">
                                ...
                                <select name="currency_id" x-model="form.currency_id" @change="setCurrency($event.target.value)" class="w-full bg-slate-50 border-slate-300 rounded-lg py-1 px-2 text-[11px] font-bold text-slate-700 focus:ring-indigo-500 h-8 text-center">
                        ...
                        {{-- TOTAL FOOTER --}}
                        <div class="mt-1.5 pt-1.5 border-t border-slate-100 flex justify-between items-center bg-slate-50 p-1.5 rounded-lg">
                    ...
                    <div class="md:col-span-5 grid grid-cols-12 gap-2 bg-white p-2.5 rounded-xl border border-slate-200 shadow-sm">
                        <div class="col-span-3">
                            <input type="text" name="statement_id" class="w-full border-slate-300 rounded-lg py-1 px-2 text-[10px] h-8 bg-slate-50 focus:bg-white text-center" placeholder="{{ __('Manual ID') }}">
                        </div>
                        <div class="col-span-3">
                            <input type="date" name="manual_date" class="w-full border-slate-300 rounded-lg py-1 px-2 text-[10px] h-8 bg-slate-50 focus:bg-white text-slate-600 text-center">
                        </div>
                        <div class="col-span-6">
                            <input name="note" class="w-full border-slate-300 rounded-lg py-1 px-2 text-[10px] h-8 bg-slate-50 focus:bg-white text-center" placeholder="{{ __('accountant.note_placeholder') }}">
                        </div>
                    </div>
