<aside id="sidebar" 
       ...
       :class="{
           ...
           'w-fit min-w-[14rem] max-w-[20rem]': !isCollapsed,
           'w-[4.5rem]': isCollapsed
       }">

    {{-- 1. HEADER --}}
    <div class="h-16 flex items-center justify-center relative border-b border-slate-800 bg-[#1e293b]/50 whitespace-nowrap overflow-hidden px-3">
    ...
    {{-- 3. MENU CONTENT --}}
    <div class="flex-1 overflow-y-auto overflow-x-hidden py-4 px-2 space-y-1.5 custom-scrollbar">
        
        <div class="px-2 mb-3 transition-all duration-300 whitespace-nowrap overflow-hidden" :class="isCollapsed ? 'text-center' : ''">
            <p x-show="!isCollapsed" class="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] px-1">{{ __('messages.main_menu') }}</p>
            <div x-show="isCollapsed" class="h-1 w-1 bg-slate-600 mx-auto rounded-full"></div>
        </div>
        ...
        <a href="{{ route('dashboard') }}" 
           ...
           :class="isCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!isCollapsed" class="text-xs font-bold whitespace-nowrap transition-opacity duration-200">{{ __('messages.dashboard') }}</span>
            <div x-show="isCollapsed" class="absolute ltr:left-14 rtl:right-14 bg-slate-900 text-white text-[10px] px-2 py-1 rounded shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-50 whitespace-nowrap border border-slate-700 font-medium">
        ...
        <x-nav-group label="{{ __('menu.define') }}" 
            ...
            <a href="{{ route('currency.index') }}" class="block px-3 py-1.5 text-[11px] font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap">{{ __('menu.currency') }}</a>
            <a href="{{ route('cash-boxes.index') }}" class="block px-3 py-1.5 text-[11px] font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap">{{ __('cash_box.title') }}</a>
            <a href="{{ route('group-spending.index') }}" class="block px-3 py-1.5 text-[11px] font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap">{{ __('spending.group_title') }}</a>
            <a href="{{ route('type-spending.index') }}" class="block px-3 py-1.5 text-[11px] font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap">{{ __('spending.type_header') }}</a>
            <a href="{{ route('profit.index') }}" class="block px-3 py-1.5 text-[11px] font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors whitespace-nowrap">{{ __('profit.groups_title') }}</a>
        </x-nav-group>

        {{-- ADMIN SECTION --}}
        @role('super-admin')
        <div class="mt-4 pt-4 border-t border-slate-800">
             <div class="px-2 mb-3 transition-all duration-300" :class="isCollapsed ? 'text-center' : ''">
                <p x-show="!isCollapsed" class="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] px-2 whitespace-nowrap">{{ app()->getLocale() == 'ku' ? 'بەڕێوەبردن' : 'ADMIN' }}</p>
                <div x-show="isCollapsed" class="h-1 w-1 bg-slate-600 mx-auto rounded-full"></div>
            </div>
    ...
    {{-- 4. FOOTER --}}
    <div class="p-3 border-t border-slate-800 bg-[#0a0f1c] space-y-2.5">
        ...
        <div class="relative w-full" x-data="{ langOpen: false }">
            <button @click="langOpen = !langOpen" 
                    ...
                    :class="isCollapsed ? 'justify-center px-0 w-10 h-10' : 'justify-between'"
                    type="button">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <span x-show="!isCollapsed" class="mx-2 truncate">{{ app()->getLocale() == 'ku' ? 'کوردی' : 'English' }}</span>
                <svg x-show="!isCollapsed" :class="langOpen ? 'rotate-180' : ''" class="w-2.5 h-2.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </button>

            {{-- Dropdown Menu --}}
            <div x-show="langOpen" 
                 class="absolute bottom-full mb-2 bg-[#1e293b] border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden min-w-[120px]"
                 :class="isCollapsed ? 'ltr:left-12 rtl:right-12' : 'w-full ltr:left-0 rtl:right-0'"
                 style="display: none;" 
                 x-cloak>
                <ul class="py-1 text-xs text-slate-300">
                    <li><a href="{{ route('lang.switch', 'en') }}" class="flex items-center w-full px-4 py-2 hover:bg-slate-700/50 hover:text-white gap-2"><span class="font-bold text-indigo-400">EN</span><span>English</span></a></li>
                    <li><a href="{{ route('lang.switch', 'ku') }}" class="flex items-center w-full px-4 py-2 hover:bg-slate-700/50 hover:text-white gap-2"><span class="font-bold text-emerald-400">KU</span><span>کوردی</span></a></li>
                </ul>
            </div>
        </div>
        ...
        <div class="relative group/user flex items-center gap-2.5 p-2 rounded-lg hover:bg-slate-800/50 transition-all duration-300 cursor-pointer border border-transparent hover:border-slate-700/50"
            ...
            <div x-show="!isCollapsed" class="flex-1 min-w-0 overflow-hidden transition-opacity duration-200">
            ...
            {{-- Collapsed Tooltip --}}
            <div x-show="isCollapsed" class="absolute ltr:left-14 rtl:right-14 bottom-0 z-50 w-max min-w-[130px] bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-2.5 opacity-0 group-hover/user:opacity-100 pointer-events-none group-hover/user:pointer-events-auto transition-all duration-300">
