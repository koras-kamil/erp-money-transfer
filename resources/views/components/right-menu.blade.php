<aside id="sidebar" class="fixed top-0 bottom-0 z-50 w-56 transition-transform duration-300 ease-in-out {{ app()->getLocale() == 'ku' ? 'translate-x-full' : '-translate-x-full' }} md:translate-x-0 bg-[#0f172a] border-slate-800 flex flex-col shadow-2xl ltr:left-0 ltr:border-r rtl:right-0 rtl:border-l">
    ...
    <div class="flex-1 overflow-visible py-4 px-2 space-y-4">
        <nav class="space-y-0.5">
            ...
            {{-- DASHBOARD --}}
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-xs font-medium {{ request()->routeIs('dashboard') ? 'bg-white text-indigo-700 font-bold shadow-lg shadow-white/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-100' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                <span>{{ __('messages.dashboard') }}</span>
            </a>

            {{-- DEFINITION GROUP (Updated Active Logic) --}}
            <x-nav-group label="{{ __('menu.define') }}" 
                         :active="request()->routeIs('currency.*') || request()->routeIs('cash-boxes.*') || request()->routeIs('group-spending.*') || request()->routeIs('type-spending.*')">
                <x-slot:icon>
                ...
                {{-- Currency --}}
                <a href="{{ route('currency.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('currency.index') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
                    {{ __('menu.currency') }}
                </a>

                {{-- Cash Box --}}
                <a href="{{ route('cash-boxes.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('cash-boxes.index') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
                    {{ __('cash_box.title') }}
                </a>

                {{-- Group Spending --}}
                <a href="{{ route('group-spending.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('group-spending.*') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
                    {{ __('spending.group_title') }}
                </a>

                {{-- Type Spending --}}
                <a href="{{ route('type-spending.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('type-spending.*') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
            ...
            <div class="mt-6 pt-6 border-t border-slate-700/50">
                ...
                <x-nav-group label="{{ app()->getLocale() == 'ku' ? 'ڕێکخستنی سیستەم' : 'System Admin' }}" 
                    ...
                    <a href="{{ route('users.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('users.*') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
                        {{ app()->getLocale() == 'ku' ? 'بەکارهێنەران' : 'Users Management' }}
                    </a>

                    <a href="{{ route('roles.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('roles.*') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
                        {{ app()->getLocale() == 'ku' ? 'دەسەڵاتەکان' : 'Roles & Permissions' }}
                    </a>

                    <a href="{{ route('activity-log.index') }}" class="block px-3 py-2 text-[11px] font-medium rounded-lg transition-colors {{ request()->routeIs('activity-log.index') ? 'bg-white text-indigo-700 font-bold shadow-md' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30' }}">
