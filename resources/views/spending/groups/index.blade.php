<x-app-layout>
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col md:flex-row justify-end items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                {{-- 1. TRASH BUTTON (Moved to Right) --}}
                <a href="{{ route('group-spending.trash') }}" title="{{ __('spending.trash') }}" 
                ...
                {{-- 3. PRINT BUTTON (PDF Link) --}}
                <a href="{{ route('group-spending.print') }}" target="_blank" title="{{ __('spending.print') }}" 
                    ...
                    <tbody id="sheet-body">
                        ...
                        <tr class="bg-white border-b border-gray-100 hover:bg-gray-50 transition-colors"
                            ...
                            <td x-show="cols.code" class="p-1">
                                <input type="text" name="spendings[{{ $index }}][code]" value="{{ $group->code }}" class="sheet-input font-bold uppercase" readonly>
                            </td>

                            {{-- Name --}}
                            <td x-show="cols.name" class="p-1">
                                <span x-show="editingId !== {{ $group->id }}" class="px-3 block text-gray-700 font-medium">{{ $group->name }}</span>
                                <input x-show="editingId === {{ $group->id }}" id="input-name-{{ $group->id }}" type="text" name="spendings[{{ $index }}][name]" value="{{ $group->name }}" class="sheet-input font-bold">
                            </td>

                            {{-- Accountant Code --}}
                            <td x-show="cols.acc_code" class="p-1">
                                <span x-show="editingId !== {{ $group->id }}" class="block text-center text-gray-600">{{ $group->accountant_code }}</span>
                            ...
                            <td x-show="cols.branch" class="p-1">
                                <span x-show="editingId !== {{ $group->id }}" class="px-3 block text-gray-600">{{ $group->branch->name ?? '-' }}</span>
                            ...
                            <td x-show="cols.actions" class="px-6 py-4 text-center print:hidden">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" @click="startEdit({{ $group->id }})" 
                                            x-show="editingId !== {{ $group->id }}"
                                            class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                    </button>
                                    <span x-show="editingId === {{ $group->id }}" class="text-emerald-500 animate-pulse">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    </span>
    ...
    <script>
        function addNewRow() {
            ...
            const userBranchId = "{{ Auth::user()->branch_id }}"; // Auto Select
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                ...
                <td x-show="cols.branch" class="p-1"><select name="spendings[${index}][branch_id]" class="sheet-input text-gray-700">${branchOptions}</select></td>
