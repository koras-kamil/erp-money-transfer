<x-app-layout>
    ...
    <div x-data="{ 
        ...
        <div id="print-header">
            ...
            <h2 class="text-lg">{{ __('spending.type_header') }}</h2>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 print:hidden px-4">
            ...
            {{-- Trash Button --}}
            <a href="{{ route('type-spending.trash') }}" class="text-xs font-bold text-red-500 hover:text-red-700 flex items-center gap-1 bg-red-50 px-3 py-1.5 rounded-full border border-red-100 transition">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-slate-500" @input="hasChanges = true" @change="hasChanges = true">
                    <thead class="text-xs text-slate-700 uppercase bg-blue-100/50 border-b border-blue-200">
                        <tr>
                            ...
                            <th x-show="cols.code" class="px-2 py-3 w-[10%]">{{ __('spending.code') }}</th>
                            <th x-show="cols.name" class="px-2 py-3 w-[20%]">{{ __('spending.name') }}</th>
                            <th x-show="cols.group" class="px-2 py-3 w-[15%]">{{ __('spending.group_title') }}</th>
                            <th x-show="cols.acc_code" class="px-2 py-3 w-[15%] text-center">{{ __('spending.accountant_code') }}</th>
                            <th x-show="cols.note" class="px-2 py-3 w-[15%]">{{ __('spending.note') }}</th>
                            <th x-show="cols.branch" class="px-2 py-3 w-[10%]">{{ __('spending.branch') }}</th>
                            <th x-show="cols.user" class="px-2 py-3 w-[5%] text-center print:hidden">{{ __('spending.created_by') }}</th>
                    ...
                    <tbody id="sheet-body" class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors group/row">
                            ...
                            <td x-show="cols.code" class="px-2 py-2">
                                <input type="text" name="types[{{ $index }}][code]" value="{{ $type->code }}" class="sheet-input font-bold uppercase text-slate-500 bg-slate-50/80 cursor-not-allowed" readonly>
                            </td>
                            ...
                            {{-- GROUP DROPDOWN --}}
                            <td x-show="cols.group" class="px-2 py-2">
                                <select name="types[{{ $index }}][group_id]" class="sheet-select w-full locked text-slate-700">
                                    <option value="">{{ __('spending.select_group') }}</option>
                                    @foreach($groups as $group)
                                        <option value="{{ $group->id }}" {{ $type->group_id == $group->id ? 'selected' : '' }}>{{ $group->name }}</option>
                                    @endforeach
                                </select>
                            </td>

                            <td x-show="cols.acc_code" class="px-2 py-2">
                                <input type="text" name="types[{{ $index }}][accountant_code]" value="{{ $type->accountant_code }}" class="sheet-input text-center" readonly>
                            </td>
                            
                            {{-- NOTE FIELD --}}
                            <td x-show="cols.note" class="px-2 py-2">
                                <input type="text" name="types[{{ $index }}][note]" value="{{ $type->note }}" class="sheet-input" readonly>
                            </td>

                            <td x-show="cols.branch" class="px-2 py-2">
                                <select name="types[{{ $index }}][branch_id]" class="sheet-select w-full locked">
                            ...
                            <td x-show="cols.user" class="px-2 py-2 text-center text-[10px] text-slate-400 print:hidden">{{ $type->creator->name ?? '-' }}</td>
    ...
    <script>
        ...
        function addNewRow() {
            ...
            @foreach($groups as $group) groupOptions += '<option value="{{ $group->id }}">{{ $group->name }}</option>'; @endforeach
            ...
            <tr class="bg-blue-50/50">
                ...
                <td x-show="cols.name" class="px-2 py-2"><input type="text" name="types[${index}][name]" class="sheet-input font-bold" placeholder="{{ __('spending.name') }}" autofocus></td>
                <td x-show="cols.group" class="px-2 py-2"><select name="types[${index}][group_id]" class="sheet-select w-full">${groupOptions}</select></td>
                <td x-show="cols.acc_code" class="px-2 py-2"><input type="text" name="types[${index}][accountant_code]" class="sheet-input text-center"></td>
                <td x-show="cols.note" class="px-2 py-2"><input type="text" name="types[${index}][note]" class="sheet-input"></td>
                <td x-show="cols.branch" class="px-2 py-2"><select name="types[${index}][branch_id]" class="sheet-select w-full">${branchOptions}</select></td>
