<x-app-layout>
    ...
    <style>
        ...
        select.sheet-input, .sheet-select {
            ...
            width: 100%; border: 1px solid transparent; padding-top: 6px; padding-bottom: 6px; border-radius: 4px; outline: none; transition: all 0.2s; font-size: 0.9rem;
        }
        
        select.sheet-input:focus, .sheet-select:focus { background-color: white; border-color: #6366f1; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.2); }
        [dir="rtl"] select.sheet-input, [dir="rtl"] .sheet-select { background-position: left 0.5rem center; padding-right: 0.75rem; padding-left: 2.5rem; }
        ...
        @media print { .no-print, button, .print\:hidden { display: none !important; } .overflow-x-auto { overflow: visible !important; } }
    </style>

    <div x-data="{ 
        hasChanges: false,
        editingId: null,
        cols: { id: true, code: true, name: true, group: true, note: true, branch: true, user: true, created_at: true, actions: true },
        ...
        <div class="mx-4 mb-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <div x-data="{ openDropdown: false }" class="relative">
                    ...
                    <div x-show="openDropdown" class="absolute top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-2 ltr:right-0 rtl:left-0" x-cloak>
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            @foreach(['id'=>'#', 'code'=>__('spending.code'), 'name'=>__('spending.name'), 'group'=>__('spending.group_title'), 'note'=>__('spending.note'), 'branch'=>__('spending.branch'), 'user'=>__('spending.created_by'), 'created_at'=>__('spending.created_at')] as $key => $label)
                ...
                <a href="{{ route('type-spending.print') }}" target="_blank" class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-700 text-white hover:bg-slate-800 transition shadow-sm"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg></a>
            ...
            <form id="sheet-form" action="{{ route('type-spending.store') }}" method="POST">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-slate-500 whitespace-nowrap">
                    <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100">
                        <tr>
                            <th x-show="cols.id" class="px-4 py-3 font-medium text-center w-[5%]">#</th>
                            <th x-show="cols.code" class="px-4 py-3 font-medium w-[10%]">{{ __('spending.code') }}</th>
                            <th x-show="cols.name" class="px-4 py-3 font-medium w-[15%]">{{ __('spending.name') }}</th>
                            <th x-show="cols.group" class="px-4 py-3 font-medium w-[15%]">{{ __('spending.group_title') }}</th>
                            <th x-show="cols.note" class="px-4 py-3 font-medium w-[15%]">{{ __('spending.note') }}</th>
                            <th x-show="cols.branch" class="px-4 py-3 font-medium w-[15%]">{{ __('spending.branch') }}</th>
                            <th x-show="cols.user" class="px-4 py-3 font-medium text-center w-[10%]">{{ __('spending.created_by') }}</th>
                            <th x-show="cols.created_at" class="px-4 py-3 font-medium text-center w-[10%]">{{ __('spending.created_at') }}</th>
                            <th x-show="cols.actions" class="px-4 py-3 font-medium text-center w-[5%] print:hidden">{{ __('spending.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody id="sheet-body" class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors group/row" :class="editingId === {{ $type->id }} ? 'bg-indigo-50/20' : ''">
                            <td x-show="cols.id" class="px-4 py-4 text-center font-normal text-slate-400">
                                {{ $loop->iteration }} <input type="hidden" name="types[{{ $index }}][id]" value="{{ $type->id }}">
                            </td>
                            ...
                            <td x-show="cols.name" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-2 block text-slate-700 font-normal truncate">{{ $type->name }}</span>
                                <input x-show="editingId === {{ $type->id }}" id="input-name-{{ $type->id }}" type="text" name="types[{{ $index }}][name]" value="{{ $type->name }}" class="sheet-input font-normal text-slate-700">
                            </td>
                            <td x-show="cols.group" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-2 block text-slate-600 font-normal">{{ $type->group->name ?? '-' }}</span>
                            ...
                            <td x-show="cols.note" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-2 block text-slate-500 text-xs font-normal truncate">{{ $type->note }}</span>
                                <input x-show="editingId === {{ $type->id }}" type="text" name="types[{{ $index }}][note]" value="{{ $type->note }}" class="sheet-input text-xs font-normal">
                            </td>
                            <td x-show="cols.branch" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-2 block text-slate-600 font-normal truncate">{{ $type->branch->name ?? '-' }}</span>
                            ...
                            <td x-show="cols.user" class="px-4 py-4 text-center text-[10px] text-slate-400 font-normal truncate">{{ $type->creator->name ?? '-' }}</td>
                            ...
                            <td x-show="cols.actions" class="px-4 py-4 text-center print:hidden">
                                <div class="relative h-8 flex items-center justify-center w-full gap-2">
                                    <button type="button" @click="saveRow()" x-show="editingId === {{ $type->id }}" class="w-7 h-7 flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg shadow-sm transition transform active:scale-95"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></button>
                                    <button type="button" @click="startEdit({{ $type->id }})" x-show="editingId !== {{ $type->id }}" class="p-1.5 text-slate-400 hover:text-blue-600 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                                    <button type="button" onclick="deleteDatabaseRow({{ $type->id }})" class="p-1.5 text-slate-400 hover:text-red-600 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
    ...
    <script>
        function addNewRow() {
            ...
            @foreach($branches as $branch) branchOptions += `<option value="{{ $branch->id }}" ${ "{{ $branch->id }}" == userBranchId ? 'selected' : '' }>{{ $branch->name }}</option>`; @endforeach
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                <td x-show="cols.id" class="px-4 py-2 text-center font-normal text-indigo-600">${nextId}</td>
                ...
                <td x-show="cols.name" class="p-1"><input type="text" name="types[${index}][name]" class="sheet-input font-normal text-slate-700" placeholder="{{ __('spending.name') }}" autofocus></td>
                ...
                <td x-show="cols.note" class="p-1"><input type="text" name="types[${index}][note]" class="sheet-input font-normal text-xs"></td>
                <td x-show="cols.branch" class="p-1"><select name="types[${index}][branch_id]" class="sheet-select w-full font-normal">${branchOptions}</select></td>
                <td x-show="cols.user" class="px-4 py-2 text-center text-[10px] text-slate-400 italic font-normal">{{ Auth::user()->name }}</td>
                <td x-show="cols.created_at" class="px-4 py-2 text-center text-xs text-indigo-400 font-mono font-normal">${dateString}</td>
                <td x-show="cols.actions" class="px-4 py-2 text-center print:hidden">
                    <div class="flex items-center justify-center gap-1">
                        <button type="button" onclick="document.getElementById('sheet-form').submit()" class="w-7 h-7 flex items-center justify-center bg-emerald-500 text-white rounded-lg shadow-sm transition transform active:scale-95"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></button>
                        <button type="button" onclick="this.closest('tr').remove()" class="text-slate-400 hover:text-red-500 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                    </div>
                </td>
            </tr>`;
