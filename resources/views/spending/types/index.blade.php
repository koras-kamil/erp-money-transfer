<x-app-layout>
    ...
    <div x-data="{ 
        ...
        cols: { select: true, id: true, code: true, name: true, group: true, note: true, branch: true, user: true, created_at: true, actions: true },
        ...
        {{-- TOOLBAR (Buttons have colors, but no container box) --}}
        <div class="mx-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                {{-- Trash (Red) --}}
                <a href="{{ route('type-spending.trash') }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-50 text-red-500 border border-red-100 hover:bg-red-100 transition shadow-sm">
                ...
                <div x-data="{ openDropdown: false }" class="relative">
                    ...
                    <div x-show="openDropdown" class="absolute top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-2 ltr:right-0 rtl:left-0" x-cloak>
                        ...
                        <div class="space-y-1 max-h-60 overflow-y-auto">
                            @foreach(['select'=>'Select', 'id'=>'#', 'code'=>__('spending.code'), 'name'=>__('spending.name'), 'group'=>__('spending.group_title'), 'note'=>__('spending.note'), 'branch'=>__('spending.branch'), 'user'=>__('spending.created_by'), 'created_at'=>__('spending.created_at')] as $key => $label)
                ...
                {{-- Print (Dark Slate) --}}
                <a href="{{ route('type-spending.print') }}" target="_blank" class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-700 text-white hover:bg-slate-800 transition shadow-sm">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-slate-500 whitespace-nowrap">
                    <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100">
                        <tr>
                            ...
                            <th x-show="cols.branch" class="px-4 py-3 font-medium min-w-[150px]">{{ __('spending.branch') }}</th>
                    ...
                    <tbody id="sheet-body" class="divide-y divide-slate-100">
                        ...
                        <tr class="bg-white hover:bg-slate-50 transition-colors group/row" 
                            ...
                            {{-- GROUP COLUMN: This displays the name when saved, or Select when editing --}}
                            <td x-show="cols.group" class="p-1">
                                <span x-show="editingId !== {{ $type->id }}" class="px-2 block text-slate-600 font-normal">
                                    {{ $type->group->name ?? '-' }}
                                </span>
                                <select x-show="editingId === {{ $type->id }}" name="types[{{ $index }}][group_spending_id]" class="sheet-input font-normal text-slate-700">
                                    <option value="">{{ __('spending.select_group') }}</option>
                                    @foreach($groups as $g)
                            ...
                            <td x-show="cols.note" class="p-1">
                                ...
                                <input x-show="editingId === {{ $type->id }}" type="text" name="types[{{ $index }}][note]" value="{{ $type->note }}" class="sheet-input font-normal text-xs">
                            </td>
                            ...
                            <td x-show="cols.branch" class="p-1">
                                ...
                                <select x-show="editingId === {{ $type->id }}" name="types[{{ $index }}][branch_id]" class="sheet-input font-normal text-slate-700">
                            ...
                            <td x-show="cols.created_at" class="px-4 py-4 text-center text-xs text-slate-400 font-mono font-normal">
                            ...
                            <td x-show="cols.actions" class="px-4 py-4 text-center print:hidden">
                                <div class="relative h-8 flex items-center justify-center w-full">
                                    ...
                                    <div x-show="editingId !== {{ $type->id }}" class="flex items-center justify-center gap-2">
                                        <button type="button" @click="startEdit({{ $type->id }})" class="text-slate-400 hover:text-blue-600 transition"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                                        <button type="button" onclick="deleteDatabaseRow({{ $type->id }})" class="text-slate-400 hover:text-red-600 transition"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    <script>
        function addNewRow() {
            ...
            // Group Options Generation
            let groupOptions = '<option value="">{{ __('spending.select_group') }}</option>';
            @foreach($groups as $g) 
            ...
            <tr class="bg-blue-50/40 border-b border-blue-100 transition-all duration-300">
                ...
                {{-- Group Select --}}
                <td x-show="cols.group" class="p-1">
                    <select name="types[${index}][group_spending_id]" class="sheet-input font-normal text-slate-700">
                        ${groupOptions}
                    </select>
                </td>
                
                <td x-show="cols.note" class="p-1"><input type="text" name="types[${index}][note]" class="sheet-input font-normal text-xs" placeholder="..."></td>
                <td x-show="cols.branch" class="p-1"><select name="types[${index}][branch_id]" class="sheet-input font-normal text-slate-700">${branchOptions}</select></td>
